...
public class CustomizeMessageKourpa2 {
	...
	// String[][] array = { { "Gkaryolakis", "Kritikos goat", "2022-12-19", "0",
	// "Sports" },
	// { "Ptheoxarhs", "MONO TEXTPAD", "2022-12-19", "100", "Music" } };
	// User user = new User();
	// User u
	...
	CustomizeMessageKourpa2(String u, String category, JPanel explore) {
		 ...
		 * 
		 * try { // Creating a new Connection Connection conn =
		 * DriverManager.getConnection(jdbcUrl); Statement statement =
		 * conn.createStatement();
		 * 
		 * //System.out.println("Connected");
		 * 
		 * // Get the right posts from the database //u.getInterest1() String query =
		 * "SELECT User.username, Post.text, Post.uploaddate, Post.likes, Post.Category FROM Post, User WHERE Post.userId = User.userId AND (Post.Category = "
		 * + "'" + category; // "'" + "COLLATE NOCASE  OR Post.Category = " + "'" +
		 * u.getInterest2() + "'" + "COLLATE NOCASE OR Post.Category = " + "'" +
		 * u.getInterest1() + "'" + "COLLATE NOCASE) ORDER BY uploadDate DESC;";
		 * 
		 * ResultSet rs = statement.executeQuery(query);
		 * 
		 * while (rs.next()) { JTextArea post = new JTextArea();
		 * post.setText(rs.getString("username") + "             Likes: " +
		 * rs.getString("likes")+ "            " + rs.getString("uploaddate") + "\n" +
		 * "\n" + "\n" + rs.getString("text") + "\n" + "\n" +"\n" + "Category:  " +
		 * rs.getString("Category")); post.setEditable(false); explore.add(post);
		 * 
		 * }
		 * 
		 * explore.setVisible(true);
		 * 
		 * } catch (SQLException s) { System.out.println("Error"); s.printStackTrace();
		 * }
		 */
		...
		try {
			...
			while (rs.next()) {
				...
				JButton like = new JButton(rs.getString("Likes"), icon);
				...
				// post.setText( array[row][0] + array [row][1] + array[row][2] + array[row][ 3]
				// + category);
		...
		/*
		 * for (int row = 0; row < array.length; row++) { // for (int col = 0; col <
		 * array[row].length ; col++) {
		 * 
		 * JPanel post = new JPanel(new BorderLayout(1, 1)); Font font = new
		 * Font("Calibri", Font.PLAIN, 20); Font fontlike = new Font("Calibri",
		 * Font.BOLD, 15); // JTextArea post = new JTextArea(5, 10);
		 * 
		 * JTextArea postUserInfo = new JTextArea(); postUserInfo.setText(array[row][0]
		 * + array[row][2] + array[row][4]); //
		 * postUserInfo.setBackground(ExploreButtons.colorscat(array[row][4]));
		 * postUserInfo.setBackground(c); postUserInfo.setEditable(false);
		 * postUserInfo.setFont(font); postUserInfo.setForeground(Color.BLACK);
		 * 
		 * JTextArea postMessage = new JTextArea(); postMessage.setText(array[row][1]);
		 * postMessage.setBackground(new Color(240, 255, 251));
		 * postMessage.setEditable(false); postMessage.setFont(font);
		 * postMessage.setForeground(Color.BLACK);
		 * 
		 * ImageIcon icon = new ImageIcon("C:\\Javaproject new\\like.png"); JButton like
		 * = new JButton("1233", icon); like.setFont(fontlike);
		 * like.setPreferredSize(new Dimension(100, 30));
		 * like.setBackground(Color.white); like.setForeground(Color.WHITE);
		 * like.setFocusable(false);
		 * 
		 * JPanel southLike = new JPanel(new FlowLayout(FlowLayout.LEFT));
		 * southLike.setSize(900, 30); southLike.setBackground(Color.white);
		 * southLike.add(like);
		 * 
		 * post.add(postUserInfo, BorderLayout.NORTH); post.add(postMessage,
		 * BorderLayout.CENTER); post.add(southLike, BorderLayout.SOUTH);
		 * explore.add(post);
		 * 
		 * // post.setText( array[row][0] + array [row][1] + array[row][2] + array[row][
		 * 3] // + category); // array[row][0].setBounds(80, 100, 100, 100);
		 * post.setBackground(c); // post.setEditable(false); post.setPreferredSize(new
		 * Dimension(1000, 1000)); // Font f = new Font("Serif", Font.SERIF, 15); //
		 * post.setFont(f); // explore.add(post); // JScrollPane scrollPane = new
		 * JScrollPane(post, //
