...
/**
 * Creates the Home page of the user according to his preferences
...
/**
*
* @param u: A parameter of type user needed to identify
* the correct posts and displays for the logged in user
* @param col: A parameter of type color
* needed to connect with other frames
* and maintain all colors throughout all of them
* @return the frame of homePage with all the components,
* which will eventually be setVisible from other classes (LogIn or Menu)
*/
...
/* To display all the tips it uses a method, which gets the
* number of the total messages from all the preferences
* the user has chosen and places them in the GridLayout rows
* parameter. The Layout has 1 column in order to display them
...
* @param frame: the homePage frame, which
* will help eventually for the visibility button
*/
...
while (rs.next()) {
/*To display each post, we use 1 panel for the
* label of the message, which contains some
* necessary information. 1 textarea for the actual
* tip and 1 panel, which contains all the functional
* buttons(like, dislike, copy, visibility)
*/
...
* @return northLabel: A JPanel representing the label of the message which contains
* all the parameters and an additional appropriate image for the category
*/
...
/**
*
* @param text:  The text area containing the main text from each post (needed for the copy button)
* @param center: The central panel where all the posts are displayed (needed for the temporary delete button)
* @param post: A parameter that defines the whole post (needed for the copy and delete buttons)
* @param frame: A parameter that takes the home page frame in order to revalidate it, when the delete button is used
* @param postid: A parameter of type int that is necessary for the like and dislike buttons
* @return the whole southern panel of each post, which contains all the functional buttons.
*/
...
* @param frame: A parameter that takes the home page frame in order to revalidate it, when the delete button is used
* @param postid: A parameter of type int that is necessary for the like and dislike buttons
* @return the whole southern panel of each post, which contains
* all the functional buttons except the temporary delete button.
*/
...
/**
*
* @param text: A parameter of type String (needed in order to get the actual text and copy it)
...
center.remove(post);
center.add(nothing);
/*This case is used when the scrollbar is no longer needed
*(applies a black space in the same size as the previous post)
*/
} else if (center.getSize().height <= 680) {
center.remove(post);
center.add(backup);
/*This case eliminates the irrelevant black space it would show by reevaluating the
* GridLayout
*/
} else {
center.remove(post);
...
* @return the total number of posts according to the preferences the user has picked
*/
...
/**
* @param categ: A parameter of type String that represents the category of each post
* @return An icon representing each category in order for the labels
* to have their own icon next to the category name
*/
...
/**
* @param categ: A parameter of type String that represents the category of each post
* @return A color representing each category in order for the labels to have their own colors
*/
...
/**
* @param c: A parameter of type int  which represents
* the number of the case chosen in the RadioButtons
* between plain mode, dark mode, light mode and color picker
*/
