...
public class CustomizeMessageKourpa2 {
	...
	// "Sports"}, {"Ptheoxarhs", "MONO TEXTPAD", "2022-12-19", "100", "Music"} };
	// User user = new User();
	// User u
		...
		try {
			...
			while (rs.next()) {
				...
				post.setText(rs.getString("username") //+ "             Likes: " + rs.getString("likes") + "            "
						+ "        Uplad Date: " + rs.getString("uploaddate") + "\n" + "\n"  + rs.getString("text") + "\n" + "\n" 
						+ "Category:  " + rs.getString("Category") + "      Likes: " + rs.getString("likes"));
				post.setEditable(false);
				post.setPreferredSize(new Dimension(1000, 1000));
				post.setForeground(Color.black);
				explore.add(post);

				JScrollPane scrollPane = new JScrollPane(post, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
